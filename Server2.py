...
def recv_local_json(conn, addr, board_path):
    ...
    while connected:
        ...
        if msg_length:
            ...
            file.write(msg)
            conn.send("Msg received json local".encode(FORMAT))
            file.close()
    conn.close()
    return
...
def handle_client_file(conn, addr, file_path):
    ...
    while connected:
        ...
        if msg_length:
            ...
            file = open(file_path, "ab")
            file.write(msg)
            conn.send("Msg received file".encode(FORMAT))
            file.close()
    conn.close()
    return
...
def handling_client(conn, addr, id): # this whole thing in a while loop
    ...
    while TRUE:
        ...
        if sys_call == 1: # getting file
            ...
            print("\n w8888888888888888888888888\n")
            set_and_read_file(conn, addr, board_path, metadata)
            print("\n 8888888888888888888 8 8 8 8\n")
            subprocess.call(['python3', '/home/david/Desktop/Backend/append.py'])
            print("[+] Concat data")
